...
四条关键脾气（2026-06-11/06-12 实测）：
  ...
  4. DStart 落在周末/节假日同样整段返回空（2026-06-12 实测周六起点空榜）——
     两端都必须是交易日。fetch_interval_all() 先把两端对齐工作日，再双向回退。

⚠️ 本文件是开盘啦区间榜的唯一实现：/fupan skill 的
   .claude/skills/fupan/scripts/kpl_interval.py 只是引用这里的薄 CLI 包装，
   接口脾气/回退逻辑改这里即可，不要两边各改各的。
...
def _snap_back_weekday(d):
    """落在周末则回退到最近的周五（脾气 #1/#4：区间端点必须是交易日）。"""
    while d.weekday() >= 5:
        d -= timedelta(days=1)
    return d
...
def _prev_weekday(d):
    return _snap_back_weekday(d - timedelta(days=1))
...
def default_range(today=None, days=5):
    ...
    DEnd 取今天（或最近工作日）：盘中含当日实时累计；不能用「昨天」当终点——
    开盘后最新交易日变成今天，不含今天的区间个股数值会被服务端脱敏（脾气 #2）。
    节假日（工作日休市）由 fetch_interval_all 的回退逻辑兜底。"""
...
def fetch_interval_all(dstart=None, dend=None, days=5):
    if not dstart or not dend:
        dstart, dend = default_range(days=days)
    # 接口对区间两端都挑剔，落在非交易日（周末/节假日/今天未开盘）整段返回空
    # （脾气 #1/#4）。先把两端对齐工作日（不花请求），节假日再双向回退——
    # 两端独立修复，不能只回退 DEnd，那会把区间塌缩成单日旧数据。
    d0 = _snap_back_weekday(datetime.strptime(dstart, "%Y-%m-%d").date())
    d1 = _snap_back_weekday(datetime.strptime(dend, "%Y-%m-%d").date())
    if d0 > d1:
        d0 = d1
    ...
    if not stocks and not sectors:
        # 1) 单日探测把 DEnd 回退到最近有数据的交易日（按工作日步进，7 步可跨长假）
        probe = 0
        while probe < 7 and not fetch_interval_stock(d1.isoformat(), d1.isoformat()):
            probe += 1
            d1 = _prev_weekday(d1)
        if d0 > d1:
            d0 = d1
        ...
        stocks = fetch_interval_stock(dstart, dend)
        sectors = fetch_interval_sector(dstart, dend)
        # 2) DStart 节假日（工作日休市）→ 向后推进到区间内首个有数据的交易日
        fallback = 0
        while not stocks and not sectors and fallback < 7 and d0 < d1:
            fallback += 1
            d0 = _next_weekday(d0)
            if d0 > d1:
                d0 = d1
